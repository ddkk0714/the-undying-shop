...
export const L = {
  ...
  live: {
    ...
    // 방송화면 액자 — 적 스프라이트 **뒤**에 깔린다 (사용자 확정).
    // 받은 그림이 462x452 라 **1:1 그대로** 놓는다. 소수배로 줄이면 액자 테두리가 뭉갠다.
    //
    // x 는 채팅창의 오른쪽 끝(782+424=1206)에 딱 붙였다. 적 상자 정중앙(1127)에 맞췄더니
    // 왼쪽 79px 이 채팅창 뒤로 숨어 액자가 잘려 보였다 (실측). 지금은 액자 네 변이 다 보이고
    // 적(1230..1486)은 액자 안에 온전히 들어간다
    screen: { x: 1206, y: 442, w: 462, h: 452 },
