...
export const L = {
  ...
  live: {
    ...
    liveBar: { x: 738, y: 148, w: 1182, h: 80 },
    badge: { x: 754, y: 151, w: 206, h: 74 },      // ON AIR 표시 (바 왼쪽 끝)
    // 목업의 채팅창은 생각보다 작다. 560 으로 넓혔더니 적 CG 자리를 먹었다 (실측)
    chat: { x: 782, y: 344, w: 424, h: 300 },
    // 적은 배경의 **바닥선**에 발을 딛는다. 배경 그림에서 길이 시작되는 높이가 y 716 근처다
    enemy: { x: 1230, y: 504, w: 256, h: 256 },
    // 우상단 흉상. **1:1 로 놓고 칸을 줄여 잘라낸다** — 배율을 낮추면(0.83배 등)
    // 도트가 지글거린다. 칸이 작아진 만큼 얼굴 위주로 더 바짝 잘린다
    portrait: { x: 1640, y: 244, w: 256, h: 248 },
    stats: { x: 1640, y: 500, w: 256, h: 252 },    // 초상 바로 아래 — 이름·공·방·체력
    lantern: { x: 1440, y: 470, w: 460, h: 568 },  // 우하단 전경 (랜턴 든 팔)
    dialogue: { x: 860, y: 800, w: 900, h: 173 },  // 용사 대사 배너 (원본 1592x307 의 비율 그대로)
