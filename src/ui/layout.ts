...
export const L = {
  ...
  live: {
    ...
    // 높이는 **절반으로 줄였다** (496 → 248, 사용자 확정). 폭 424 는 `ui.live.chat` 원본과
    // 같은 값이라 9-slice 가 가로로는 늘어나지 않는다.
    // 보이는 줄은 (248 - 80) / 32 ≈ 5줄이다 — core 는 그대로 13개를 살려 두므로
    // 오래된 것부터 화면 밖으로 밀려난다 (채팅이 원래 그렇게 흐른다)
    chat: { x: 782, y: 216, w: 424, h: 248 },
    // 슈퍼챗 팝업 — 채팅창 오른쪽에 잠깐 떴다 사라진다. 원본 383x62 를 1:1 로 놓는다
    superchat: { x: 1226, y: 220, w: 383, h: 62 },
    ...
    // **두 줄이 들어가야 한다** (사용자 확정). 900x173 일 때는 한 줄만 들어가서
    // 긴 대사가 「…·」로 잘렸다. 대사집에서 가장 긴 던전 줄이 32px 기준 1136px 이라
    // 글자 자리 폭이 700px 쯤 되면 두 줄에 담긴다 → 상자 폭 1100.
    // 아래 변(956)은 3택의 위 변과 같게 맞춘다 — 둘이 한 덩어리다
    dialogue: { x: 779, y: 716, w: 1100, h: 240 },
