...
**정상 플레이에서 도달 가능한지는 단정하지 않는다** — 굿즈 수입이 매일 들어오고 소생 비용은
층·열화에 따라 오르니 밸런스 문제다. 다만 **도달하면 게임이 끝나지도 진행되지도 않는다.**
심사 중에 이 상태에 빠지면 그 판은 거기서 끝난다.

**제안** (어느 쪽이든 core 몫이다):
- 출연자가 0명이면 지원자가 반드시 한 명 찾아온다 (`recruitPool` 이 비어도 최소 1명)
- 또는 골드가 모자라도 「빚을 지고 소생」을 허용한다 (평판·leak 대가)
- 또는 **파산 엔딩**을 하나 만든다. 막다른 길보다 엔딩이 낫다

화면 쪽은 어느 쪽이든 버튼 하나 붙이면 된다. 규칙만 정해 달라.
