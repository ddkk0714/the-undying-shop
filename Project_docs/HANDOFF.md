...
## 2. ★ 지금 게임을 막고 있는 것 — HO-010

**실제 사망이 시체를 만들지 않는다.** 8일을 완주해도 시체 0구 · 골드 12840 고정 · 아무도 안 죽는다.
`reducer.ts` `finishLive()` 한 줄이 원인이고 **Codex 소유라 내가 못 고친다.** 위 HO-010 참조.

> **이게 안 고쳐지면 M08(사망 기록)·M09(검시)·M10(경제) 화면은 만들어도 빈 화면만 나온다.**
> 다음 세션이 검시·발표 화면을 붙잡기 전에 **HO-010 이 처리됐는지 먼저 확인해라.**
> 확인법: 편성실에서 출격 → 죽을 때까지 두고 → `window.__store.getState().corpses.length` 가 1 이상인가.
