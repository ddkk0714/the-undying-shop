...
| 키 패턴 | 더미 형태 (v3.1 · 1920×1080 기준) |
|---|---|
| `bg.*` | 1920×936 `ink` 바탕 + 50% Bayer 디더 + 좌상단 키 이름 |
| `ui.*.9s` | `mid` 채움 + `bone` 2px 테두리 9-slice |
| `star.portrait.*` | 384×480 `mid` 실루엣 + 이니셜 |
| `sfx.*` | 0.2초 무음 ogg |

**중요**: 플레이스홀더도 **팔레트 5토큰만** 쓴다. 그래야 아트 교체 시 레이아웃 충격이 없다.

### 4-1. 본 아트 규격 (v3.1 — 1비트 디더)

- 캔버스 **1920×1080**, 색은 `ink` / `bone` 2톤 + 캐릭터 중간 면적에만 `mid`
- 그라디언트·소프트 셰이딩 금지. 명암은 **오더드 디더(Bayer 4×4 / 8×8)** 로만
- 디더 격자는 **화면 좌표 기준으로 정렬**한다. 스프라이트마다 위상이 어긋나면 지저분해진다
- 배경은 램프 같은 **광원 1개**를 두고 방사형 디더로 감쇠시킨다 (레퍼런스 작업대)
- 외곽선은 `bone` 2px 하드. 안티앨리어싱된 픽셀이 한 개도 없어야 한다
- 붉은색(`wax`)은 **UI 강조에만**. 배경 아트에는 쓰지 않는다
...
데모는 `ko`만 채우되 **구조만 열어둔다.** `Project_docs/modules/M12-platform-deploy.md`의 Hive 섹션과 연결되는 근거가 된다.
